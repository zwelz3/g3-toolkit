...
- Residual Windows gate failure (verify:types EINVAL) fixed at the class level. Both scripts/verify-types.mjs and scripts/check-readme-snippets.mjs invoked TypeScript through the node_modules/.bin shim, which on Windows is tsc.cmd; patched Node (the CVE-2024-27980 mitigation) refuses to spawn .cmd/.bat without a shell, so execFileSync threw EINVAL regardless of the scripts' existing win32 branch. Both now spawn process.execPath with typescript/lib/tsc.js resolved via createRequire against the repo root: shell-free, quoting-hazard-free, and identical on every platform. The maintainer had only hit verify:types (the chain stops there); verify:snippets carried the same defect one stage later and is fixed preemptively. Process note recorded because it is the second occurrence of the pattern: the first fix attempt silently modified NOTHING (an anchor assertion failed after the first file's import edit, before any write) while both stages still reported green locally, since Linux tolerates the .bin spawn; the corrected edit was verified by grepping for createRequire in both files, not by stage color. Windows re-run is the confirmation.

- Visual Acceptance surface RETIRED per maintainer directive (it was a portable validation artifact for the development period, not a product surface). Removed: scripts/visual-acceptance/ itself, both vite configs (vite.va.config.ts, vite.va-client.config.ts), the visual-acceptance package script, the docs:build steps that built and copied it into docs-out/acceptance, the docs landing card, and the two .gitignore entries; planning/visual-acceptance-1.md and the checklist moved to planning/archive/ per the flagship precedent; CLAUDE.md and STATUS.md references annotated RETIRED 2026-07-04 (the visual review channel is now the Pages playground, plus Playwright screenshots once baselines land); the docs.yml comment updated. The VA-26 runaway-growth probe imported island components from the deleted surface; the regression class it guards (React state feedback loops in canvas-plus-panels compositions) is real and surface-agnostic, so the probe was RETARGETED rather than deleted: tests/unit/render-settle-probe.test.tsx mounts the living AnalyticsDashboard with the painting leaves stubbed (canvas, StatsPanel, LinkedChart; echarts wakes during the settle window and disposes against jsdom's null 2D context) and counts COMMITTED renders via a dep-array-less effect (react-hooks/immutability correctly rejects mutating module state during render; commit-counting is StrictMode-stable and detects the same unbounded-loop class). Suite: 113 files / 1072 tests (down exactly one test: the old probe had two retired subjects, the successor one). Local docs evidence: docs:demo builds and the edited landing-copy step executes with zero acceptance references in the deployed index; the full docs:build (typedoc plus storybook) validates in CI via docs.yml on the next push.
